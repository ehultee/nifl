...
# -*- coding: utf-8 -*-
"""
Isolate short term (full signal-longterm)
Compare xcorr with that for full signals

Created on Fri Mar 26 14:45:35 2021
...
def Xcorr1D_st(pt, series_func, series_dates, velocity_pred, t_grid, t_limits, 
    ...
    isolating for short-term variability
    ...
    vel_shortterm = velocity_pred['full'] - (velocity_pred['secular'] + velocity_pred['transient'])
    series_diff = np.diff(coincident_series, n=diff)
    vel_series_0 = vel_shortterm[np.where(t_grid>=t_min)]
...
tm_residual = tm_evensampled - tm_filtered
tf_highfreq = interpolate.UnivariateSpline(t_grid_trimmed, tm_residual, s=0)


term_st_corr_amax = []
term_st_lag_amax = []
for xy, pred in zip(xys, preds):
    corr, lags, ci = Xcorr1D_st(xy, series_func=tf_highfreq, series_dates=tm_d_interp, 
                              velocity_pred=pred, t_grid=t_grid, t_limits=(2009,2017), 
                              diff=1, normalize=True)
    term_st_corr_amax.append(corr[abs(corr).argmax()])
    term_st_lag_amax.append(lags[abs(corr).argmax()])
...
rnf_residual = rnf_evensampled - rnf_filtered
rf_highfreq = interpolate.UnivariateSpline(t_grid_trimmed, rnf_residual, s=0)

rf_st_corr_amax = []
rf_st_lag_amax = []
for xy, pred in zip(xys, preds):
    corr, lags, ci = Xcorr1D_st(xy, series_func=rf_highfreq, series_dates=d_interp, 
                              velocity_pred=pred, t_grid=t_grid, t_limits=(2009,2017), 
                              diff=1, normalize=True, pos_only=True)
    rf_st_corr_amax.append(corr[abs(corr).argmax()])
    rf_st_lag_amax.append(lags[abs(corr).argmax()])
...
smb_residual = smb_evensampled - smb_filtered
smb_highfreq = interpolate.UnivariateSpline(t_grid_trimmed, smb_residual, s=0)

smb_st_corr_amax = []
smb_st_lag_amax = []
for xy, pred in zip(xys, preds):
    corr, lags, ci = Xcorr1D_st(xy, series_func=smb_highfreq, series_dates=smb_d_interp, 
                              velocity_pred=pred, t_grid=t_grid, t_limits=(2009,2017), 
                              diff=1, normalize=True, pos_only=True)
    smb_st_corr_amax.append(corr[abs(corr).argmax()])
    smb_st_lag_amax.append(lags[abs(corr).argmax()])
...
sc1 = ax1.scatter(np.asarray(xys)[:,0], np.asarray(xys)[:,1], c=smb_st_corr_amax, cmap=div_colors,
...
sc2 = ax2.scatter(np.asarray(xys)[:,0], np.asarray(xys)[:,1], c=rf_st_corr_amax, cmap=div_colors,
                  vmin=corrnorm_min, vmax=corrnorm_max)
# ## set up correctly scaled colorbar
...
sc3 = ax3.scatter(np.asarray(xys)[:,0], np.asarray(xys)[:,1], c=term_st_corr_amax, cmap=div_colors,
                  vmin=corrnorm_min, vmax=corrnorm_max)
...
sc4 = ax4.scatter(np.asarray(xys)[:,0], np.asarray(xys)[:,1], c=smb_st_lag_amax, cmap=lag_colors,
...
sc5 = ax5.scatter(np.asarray(xys)[:,0], np.asarray(xys)[:,1], c=rf_st_lag_amax, cmap=lag_colors,
                  vmin=lagnorm_min, vmax=lagnorm_max)
# ## set up correctly scaled colorbar
...
sc6 = ax6.scatter(np.asarray(xys)[:,0], np.asarray(xys)[:,1], c=term_st_lag_amax, cmap=lag_colors,
                  vmin=lagnorm_min, vmax=lagnorm_max)
## set up correctly scaled colorbar
